const Game = {
    ...
    buyItem(id) { 
        ...
        if (this.player[currency] >= it.price) {
            ...
            // 特殊处理：如果是技能书，且没学过，直接学习并尝试佩戴
            if (it.type === 'book') {
                if (this.player.skills.includes(it.skillId)) {
                    alert("已学会此功法！(货币已退回)");
                    this.player[currency] += it.price;
                    return;
                }
                this.player.skills.push(it.skillId);
                // 自动佩戴
                if (this.player.equippedSkills.length < 3) {
                    this.player.equippedSkills.push(it.skillId);
                    this.log(`购买并装备 [${it.name}]`, 'win');
                } else {
                    this.log(`购买并习得 [${it.name}]`, 'win');
                }
            } else {
                this.addItem(id, 1);
        ...
        this.save(); this.recalcStats(); this.updateUI();
    ...
    // --- 核心修复：战斗循环 ---
    loop() {
        // 如果正在战斗，不要重复启动，防止多重定时器
        if (this.isFighting) return; 
        
        let e;
        try {
            if (this.currentMap === 'boss' && this.currentBoss) {
                e = { ...this.currentBoss, hp: this.currentBoss.hp, loot: this.currentBoss.drops, daoyun: this.currentBoss.daoyun };
        ...
        // 启动战斗
        Battle.start(this.player, e, this.player.equippedSkills,
            // 胜利回调
            () => { 
                this.isFighting = false; // 标记战斗结束
                this.gainReward(e);
            ...
            () => { 
                ...
                setTimeout(() => this.loop(), 2000); // 失败休息2秒
            },
    ...
    useItem(key) {
        ...
        if (item.type === "crystal") {
            ...
            this.log(`强化武器 +${item.boost}`, 'win');
            this.player.inventory[key]--;
        }
        else if (item.type === "exp") {
            this.player.exp += item.effect.val;
            this.log(`服用${item.name} +${item.effect.val}修为`, 'win');
            this.player.inventory[key]--;
        }
    ...
    toggleCoupon(id) {
        this.player.activeCoupon = (this.player.activeCoupon === id) ? null : id;
        this.save(); this.refreshPanel();
    },

    toggleSkill(skillId) {
        const idx = this.player.equippedSkills.indexOf(skillId);
        if (idx > -1) this.player.equippedSkills.splice(idx, 1);
        else {
            if (this.player.equippedSkills.length >= 3) { alert("最多装备3个功法"); return; }
            this.player.equippedSkills.push(skillId);
        }
        this.recalcStats(); this.save(); this.refreshPanel();
    },

    gainReward(e) {
        let mult = 1;
        if (this.player.activeCoupon) {
            if ((this.player.inventory[this.player.activeCoupon]||0) > 0) {
                mult = GAME_DATA.items[this.player.activeCoupon].mult;
                this.player.inventory[this.player.activeCoupon]--;
                if(this.player.inventory[this.player.activeCoupon]<=0) { delete this.player.inventory[this.player.activeCoupon]; this.player.activeCoupon = null; }
            } else this.player.activeCoupon = null;
        }
        ...
        this.player.exp += exp; this.player.money += money; this.player.daoyun += daoyun;
        
        if (e.loot) {
            e.loot.forEach(k => { 
                this.addItem(k, 1);
                if(mult > 1 && Math.random() < (mult-1)) this.addItem(k, 1);
                this.log(`获得: [${this.getItemName(k)}]`, 'win');
            });
        }
        let msg = `胜! 修为+${exp} 灵石+${money}`;
        if(daoyun) msg += ` 道韵+${daoyun}`;
        this.log(msg, 'win');
    },
    ...
    refreshPanel() {
        ...
        for(let s in GAME_DATA.equipSlots){
            ...
            if(eq){
                let txt = `${this.getItemName(eq.id)}`; if(s==='weapon'&&eq.enhanceAtk) txt+=`(+${eq.enhanceAtk})`;
        ...
        for(let [k,c] of Object.entries(this.player.inventory)){
            ...
            else if(it) { d=it.desc; if(it.type==='exp'||it.type==='crystal')btn=`<button class="item-btn btn-buy" onclick="Game.useItem('${k}')">使用</button>`; if(it.type==='coupon'){const act=this.player.activeCoupon===k; btn=`<button class="item-btn ${act?'btn-sell':'btn-buy'}" onclick="Game.toggleCoupon('${k}')">${act?'停止':'开启'}</button>`;}}
        ...
        if(sb){
            if(this.player.skills.length===0) sb.innerHTML='<div style="color:#666;font-size:0.8em">暂无</div>'; 
                ...
                this.player.skills.forEach(sid => { 
                    const sk = GAME_DATA.skills[sid]; const eq = this.player.equippedSkills.includes(sid);
                    if(sk) html += `<div class="skill-tag ${eq?'active':''}" onclick="Game.toggleSkill('${sid}')">${sk.name}${eq?'✅':''}</div>`;
                });
                sb.innerHTML = html;
            }
        }
    ...
    synthesizeAll() { let ok=false; for(let k in this.player.inventory){if(k.includes('_')&&!k.startsWith('crystal')&&this.player.inventory[k]>=3){const [t,tr]=k.split('_');const tier=parseInt(tr);if(tier<20){const n=Math.floor(this.player.inventory[k]/3);this.player.inventory[k]%=3;if(this.player.inventory[k]===0)delete this.player.inventory[k];this.addItem(`${t}_${tier+1}`,n);ok=true;}}else if(k.startsWith('crystal_')&&this.player.inventory[k]>=3){const tier=parseInt(k.split('_')[1]);if(tier<5){const n=Math.floor(this.player.inventory[k]/3);this.player.inventory[k]%=3;if(this.player.inventory[k]===0)delete this.player.inventory[k];this.addItem(`crystal_${tier+1}`,n);ok=true;}}} if(ok){this.save();this.refreshPanel();}else alert("材料不足"); },
    ...
    sellItem(k) { if(!this.player.inventory[k])return; let p=5; if(k.includes('_')&&!k.startsWith('crystal'))p=parseInt(k.split('_')[1])*50; else if(GAME_DATA.items[k])p=GAME_DATA.items[k].price/2; this.player.money+=Math.floor(p); this.player.inventory[k]--; if(this.player.inventory[k]<=0)delete this.player.inventory[k]; this.save();this.updateUI();this.refreshPanel(); },
