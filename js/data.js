const GAME_DATA = {
    ...
    fieldMonsters: [
        { level: 1, name: "野兔精", hpMult: 1, atkMult: 0.5, exp: 8, money: 2, loot: ["weapon_1", "body_1"] }, 
        { level: 5, name: "狂暴野猪", hpMult: 4, atkMult: 1.2, exp: 25, money: 10, loot: ["head_1", "pants_1"] },
    ...
    maps: {
        ...
        boss: { name: "上古禁地", genEnemy: (p) => ({ name: "远古魔像", hp: p.maxHp * 6, atk: p.def * 1.8 + 60, def: p.atk * 0.25, exp: 6000, money: 3000, loot: ["weapon_4", "body_4"] }) },
        tower: { name: "镇妖塔", genEnemy: (p, f) => {
            const s = Math.pow(1.12, f);
            return { name: `第${f}层 守卫`, hp: Math.floor(250 * s), atk: Math.floor(25 * s), def: Math.floor(12 * s), pen: Math.floor(6 * s), exp: Math.floor(60 * s), money: Math.floor(30 * s) };
        }}
    }
};
...
(function() {
    ...
    GAME_DATA.majorRealms.forEach((name, idx) => {
        ...
        for (let i = 1; i <= 10; i++) {
            bExp = Math.floor(bExp * 1.18 + 200); // 经验需求增长
            if (i === 1) sMult *= 1.6; else sMult *= 1.07; // 大境界跃升，小境界微调
            GAME_DATA.realms.push({ 
                name: `${name}${i === 10 ? '圆满' : i + '层'}`, 
                exp: bExp, 
                mult: parseFloat(sMult.toFixed(2)), 
                isMajor: i === 1 
            });
        }
    });
