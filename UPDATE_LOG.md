...
- [ ] ## [V1.9.0] - 2026-02-22
### 核心变更
- **法宝属性公式**：Artifact_Atk = Tier_Base * (1 + Quality_Bonus) * (1 + Level * 0.1)。
- **词条逻辑**：
    - 词条采用百分比加成，存放在 `state.artifact.affixes` 数组中。
    - 洗炼操作会重新生成数组中特定索引的随机对象。

### 🚩 当前逻辑锚点
- 只有当前 Level 为 10 时，UI 才会显示“提升品质”或“升阶”按钮。
- 升阶逻辑会触发 `ArtifactSystem.generateAffix()`，为法宝注入新的随机属性。

### 🚀 TODO
- [ ] 编写 `/systems/artifact.js`。
- [ ] 在 `data.json` 中为法宝配置各个阶位的 UI 颜色。
